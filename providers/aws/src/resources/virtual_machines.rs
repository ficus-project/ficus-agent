...
impl VirtualMachineProvider for AwsVirtualMachineProvider {
      ...
      match get_ec2(&self.ec2_client, next_token).await {
        Ok((ec2_instances, token)) => {
          for ec2_instance in ec2_instances {
            let instance = self.map_ec2_to_model(ec2_instance);
  ...
  async fn measure_virtual_machines_usage(&self, identifiers: &Vec<String>, from_timestamp: u64, to_timestamp: u64) -> Result<Box<HashMap<String, HashMap<String, UsageMetric>>>, FetchResourceError> {
      ...
      match get_ec2_cpu_usage(&self.cloudwatch_client, &identifier, DateTime::from_secs(from_timestamp as i64), DateTime::from_secs(to_timestamp as i64), 3600).await {
        Ok(datapoints) => {
          for datapoint in datapoints {
            ...
            let mut metrics_hashmap = HashMap::new();
            metrics_hashmap.insert(String::from("vm_cpu"), UsageMetric { timestamp, average: datapoint.average(), sum: datapoint.sum() });
            metrics.insert(identifier.clone(), metrics_hashmap);
          }
        },
        Err(error) => {
...
impl AwsVirtualMachineProvider {
  fn map_ec2_to_model(&self, ec2_instance: Instance) -> VirtualMachine {
    ...
    VirtualMachine { identifier, cpu_cores, cpu_threads, memory_in_mb, is_running, tags, provider: String::from(PROVIDER_IDENTIFIER) }
  }
}
